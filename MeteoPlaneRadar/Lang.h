...
// X(id, czech DISP (ASCII), czech WEB (UTF-8), slovak DISP (ASCII), slovak WEB (UTF-8), english)
//
...
#define LANG_STRINGS(X) \
  ...
  X(S_TACTICAL,      "Takticky radar",     "Taktický radar",     "Takticky radar",     "Taktický radar",     "Tactical radar") \
...
void    Lang_Set(uint8_t lang);     // LANG_CZ / LANG_EN / LANG_SK; anything else = CZ
uint8_t Lang_Get();
